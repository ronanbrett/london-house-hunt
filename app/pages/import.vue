...
const bookmarklet = computed(
  () =>
    `javascript:(function(){var p=null,d=null;if(window.PAGE_MODEL){p='rightmove';d=window.PAGE_MODEL;}else{var z=document.getElementById('__ZAD_TARGETING__');if(z){p='zoopla';var t=null,j=null;try{t=JSON.parse(z.textContent);}catch(e){}var ls=document.querySelectorAll('script[type="application/ld+json"]');for(var i=0;i<ls.length;i++){try{var ld=JSON.parse(ls[i].textContent);if(ld&&ld['@type']==='RealEstateListing'){j=ld;break;}}catch(e){}}d={targeting:t,jsonLd:j};}}if(!d){alert('No listing data found on this page.');return;}fetch('${base}/api/properties/import-bookmarklet',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({portal:p,data:d,url:location.href})}).then(function(r){return r.json();}).then(function(j){if(j&&j.id){if(confirm('Saved to House-Hunt. Open it?'))location.href='${base}/properties/'+j.id;}else alert('Import failed.');}).catch(function(){alert('Could not reach the House-Hunt app.');});})();`,
)
